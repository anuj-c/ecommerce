...
// GETTING ALL TABLES
$tables = $conn->query("SELECT Table_name as TableName from information_schema.tables where table_schema = 'ecommerce' AND table_comment <> 'private';");
?>
<div class="container">
    ...
    <form action="upload/nav.php" method="POST" class="tableNames">
        <?php if ($tables->num_rows > 0) { ?>
            <?php while ($row = $tables->fetch_assoc()) { ?>
                <input type="submit" value="<?php echo $row['TableName']; ?>" name="table" class="container my-2 btn btn-primary py-3 pl-5">
        <?php }
        } ?>
    </form>
    <?php }
